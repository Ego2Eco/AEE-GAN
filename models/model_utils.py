...
class ModelSelector():
    ...
    def get_model(self, model_name, dataset_name, evaluate_result_dir, model_checkpoints_dir):
        val_file_name = os.path.join(evaluate_result_dir, "val.txt")
        f_val = open(val_file_name, "r")
        self.save_evl_result(f_val.readlines())

        self.sort_evl_result()
        epoch = int(self.evl_result_list[4][0])
        avg_ade = float(self.evl_result_list[4][1])
        avg_fde = float(self.evl_result_list[4][2])
        min_ade = float(self.evl_result_list[4][3])
        min_fde = float(self.evl_result_list[4][4])

        model_path = model_checkpoints_dir + '/' + model_name + '_' + dataset_name + \
                     '_' + str(epoch) + '.pt'

        print('Selected model is %d epoch. \n Avg ADE,FDE (12)= (%.3f, %.3f) | Min(20) ADE,FDE (12)= (%.3f, %.3f)' \
               % (epoch, avg_ade, avg_fde, min_ade, min_fde))
