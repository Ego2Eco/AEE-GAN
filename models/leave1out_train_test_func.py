...
def train(epoch):
    ...
    for ii, batch_i in enumerate(train_data['the_batches']):
        batch_size_accum += batch_i[1] - batch_i[0]
        sub_batches.append(batch_i)
        frame_indexes.append(ii)

        if ii >= train_size - 1 or \
                batch_size_accum + (train_data['the_batches'][ii + 1][1] - train_data['the_batches'][ii + 1][0]) > batch_size:
            
            bs = batch_size_accum
            # Slice the batch portion from all data
            batch_ids = train_data['dataset_agent_ids'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_obsv_5d = train_data['dataset_obsv_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_pred_5d = train_data['dataset_pred_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_fcn_features = train_fcn_features[frame_indexes[0]:frame_indexes[-1]+1]
...
def evaluate(epoch):
    ...
    for ii, batch_i in enumerate(val_data['the_batches']):
        batch_size_accum += batch_i[1] - batch_i[0]
        ...
        if ii >= val_size - 1 or \
                batch_size_accum + (val_data['the_batches'][ii + 1][1] - val_data['the_batches'][ii + 1][0]) > batch_size:
            
            bs = batch_size_accum
            # Slice the batch portion from all data
            batch_ids = val_data['dataset_agent_ids'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_obsv_5d = val_data['dataset_obsv_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_pred_5d = val_data['dataset_pred_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_fcn_features = val_fcn_features[frame_indexes[0]:frame_indexes[-1]+1]
...
def test():
    ...
    best_model_path = model_selector.get_best_model(model_name, dataset_name, evaluate_result_dir, model_checkpoints_dir)
    ...
    for ii, batch_i in enumerate(test_data['the_batches']):
        batch_size_accum += batch_i[1] - batch_i[0]
        ...
        if ii >= test_size - 1 or \
                batch_size_accum + (test_data['the_batches'][ii + 1][1] - test_data['the_batches'][ii + 1][0]) > batch_size:
            
            bs = batch_size_accum
            # Slice the batch portion from all data
            batch_ids = test_data['dataset_agent_ids'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_obsv_5d = test_data['dataset_obsv_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_pred_5d = test_data['dataset_pred_5d'][sub_batches[0][0]:sub_batches[-1][1]]
            batch_fcn_features = test_fcn_features[frame_indexes[0]:frame_indexes[-1]+1]
